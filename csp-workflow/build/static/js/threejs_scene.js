...
const keyState = {};
const cameraHome = { position: new THREE.Vector3(), target: new THREE.Vector3() };
...
function animateQueue(elapsed) {
  ...
  queueBoxes.forEach((entry, index) => {
    const loopLen = 28;
    const offset = (elapsed * speed + index * 0.6) % loopLen;
    entry.mesh.position.x = -15 + offset;
  });
}
...
function syncAgents(agents, layout) {
  ...
  agents.forEach((agent) => {
    const seat = layout.positions.find((p) => p.agent_id === agent.id);
    if (!seat) return;
...
function updateQueue(backlogCount, beltSpeed, arrivalRate, throughputRatio) {
  ...
  const inflowUnits = Math.max(Math.round(arrivalRate * 6), 6);
  const displayCount = Math.min(backlogCount + inflowUnits, 30);
  for (let i = 0; i < displayCount; i++) {
    ...
    const mat = new THREE.MeshStandardMaterial({
      color,
      emissive: isBacklog ? 0x1a0a0a : 0x082226,
    });
    const cube = new THREE.Mesh(geo, mat);
    cube.position.set(-15 + i * 1.3, 0.8, -10);
    queueGroup.add(cube);
    queueBoxes.push({ mesh: cube, backlog: isBacklog });
  }
...
async function runScenario(modeShortcut) {
  const payload = {
...
function updateMetricsPanel(data) {
  ...
  const label = scenarioMeta?.label || data.scenario.label;
  ...
  const utilPercent = Math.round((data.stats.average_utilization || 0) * 100);
  const throughputPct = Math.round((data.metrics.throughput_ratio || 0) * 100);
  const completionRate =
    data.parameters.max_minutes > 0 ? data.metrics.processed_minutes / data.parameters.max_minutes : 0;
  const flowDelta = data.metrics.arrival_rate - completionRate;

  metricUtil.textContent = `${utilPercent}%`;
  metricBacklog.textContent = data.backlog.total_minutes.toFixed(0);
  metricBacklogCount.textContent = data.backlog.count;
  metricArrival.textContent = `${data.metrics.arrival_rate}/min`;
  metricThroughput.textContent = `${throughputPct}%`;
  metricAssistants.textContent = `${data.agents.length} / ${data.parameters.requested_agents}`;
  metricFlow.textContent = `${flowDelta >= 0 ? "+" : ""}${flowDelta.toFixed(2)}/min`;
  metricBelt.textContent = `${data.metrics.belt_speed.toFixed(1)}x`;
  updateProductionCycle(throughputPct, data.backlog.count);
  renderAssistantsPanel(data.agents);
}
...
function setLoading(isLoading) {
  ...
  spikeBtn.disabled = isLoading;
  playBtn.textContent = isLoading ? "Running..." : "Play";
}
...
function animateAgents(elapsed) {
  ...
  agentMeshes.forEach((mesh) => {
    ...
    const speed = 1 + util * 1.2;
    const wobble = Math.sin(elapsed * speed + offset) * wobbleAmp;
    mesh.position.y = (mesh.userData.baseY || 1.2) + wobble;
    mesh.rotation.z = Math.sin(elapsed * 0.8 + offset) * wobbleAmp * 0.6;
  });
}
...
function renderAssistantsPanel(agents = []) {
  ...
  agents
    ...
    .forEach((agent) => {
      ...
      const avg = agent.tasks && agent.tasks.length ? agent.total_time / agent.tasks.length : 0;
      const avgLabel = formatDuration(avg);
      row.innerHTML = `<strong>A${agent.id}</strong><span>${avgLabel} / task</span>`;
